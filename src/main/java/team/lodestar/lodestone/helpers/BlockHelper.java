...
public class BlockHelper {
     ...
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int range, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, range, predicate).collect(Collectors.toSet());
    }

    /**
     * Returns a list of block entities within a range, with a predicate for conditional checks, as stream
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int range, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, range, range, range, predicate);
    }

    /**
     * Returns a list of block entities within an XZ range, with a predicate for conditional checks.
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int x, int z, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, x, z, predicate).collect(Collectors.toSet());
    }

    /**
     * Returns a list of block entities within an XZ range, with a predicate for conditional checks, as stream
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int x, int z, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, x, z).filter(predicate);
    }

    /**
     * Returns a list of block entities within an XYZ range, with a predicate for conditional checks.
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int x, int y, int z, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, x, y, z, predicate).collect(Collectors.toSet());
    }

    /**
     * Returns a list of block entities within an XYZ range, with a predicate for conditional checks, as streamq
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int x, int y, int z, Predicate<T> predicate) {
        return getBlockEntitiesStream(type, world, pos, x, y, z).filter(predicate);
    }

    /**
     * Returns a list of block entities within a radius around a position.
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int range) {
     ...
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int range) {
        return getBlockEntitiesStream(type, world, pos, range, range, range);
    }

    /**
     * Returns a list of block entities within an XZ radius around a position.
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int x, int z) {
        return getBlockEntitiesStream(type, world, pos, x, z).collect(Collectors.toSet());
    }
     ...
     * @return - Stream of block entities
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int x, int z) {
        return getBlockEntitiesStream(type, world, new Box((double) pos.getX() - x, pos.getY(), (double) pos.getZ() - z, (double) pos.getX() + x, (double) pos.getY() + 1, (double) pos.getZ() + z));
    }
     ...
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int x, int y, int z) {
        return getBlockEntitiesStream(type, world, pos, x, y, z).collect(Collectors.toSet());
    }
     ...
     * @return - Stream of block entities
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int x, int y, int z) {
        return getBlockEntitiesStream(type, world, pos, -x, -y, -z, x, y, z);
    }
     ...
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, BlockPos pos, int x1, int y1, int z1, int x2, int y2, int z2) {
        return getBlockEntitiesStream(type, world, pos, x1, y1, z1, x2, y2, z2).collect(Collectors.toSet());
    }

    /**
     * Returns a list of block entities within set coordinates, as stream
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, BlockPos pos, int x1, int y1, int z1, int x2, int y2, int z2) {
        return getBlockEntitiesStream(type, world, new Box((double) pos.getX() + 1.5f +  x1, (double) pos.getY() + 1.5f + y1, (double) pos.getZ() + 1.5f + z1, (double) pos.getX() + 0.5f + x2, (double) pos.getY() + 0.5f + y2, (double) pos.getZ() + 0.5f + z2));
    }

    /**
     * Returns a list of block entities within an AABB.
     */
    public static <T> Collection<T> getBlockEntities(Class<T> type, World world, Box box) {
        return getBlockEntitiesStream(type, world, box).collect(Collectors.toSet());
    }

    /**
     * Returns a list of block entities within an AABB, as stream
     */
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World world, Box box) {

        // Int stream from min to max + 16 (to get all chunks), step 16
        return IntStream.iterate((int) Math.floor(box.minX), i -> i < Math.ceil(box.maxX) + 16, i -> i + 16)
                .boxed()
                .flatMap(i ->
                        // Int stream from min to max + 16 (to get all chunks), step 16
                        IntStream.iterate((int) Math.floor(box.minZ), j -> j < Math.ceil(box.maxZ) + 16, j -> j + 16)
                                .boxed().flatMap(j -> {
                                    // get chunk access
                ...
                .filter(p -> box.contains(p.getX() + 0.5, p.getY() + 0.5, p.getZ() + 0.5))
     ...
     * */
    public static Collection<BlockPos> getPath(BlockPos start, BlockPos end, int speed, boolean inclusive, World world) {
        ...
        for (int i = 0; i < nodes; i++) {
            ...
            positions.add(BlockPos.ofFloored(node.x, node.y - 0.5, node.z));
        }
        if (!inclusive) {
            positions.remove(0);
